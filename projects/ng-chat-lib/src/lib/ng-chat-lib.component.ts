...
export class NgChatLibComponent implements OnInit, IChatController {
  ...
  onParticipantClickedFromFriendsList(participant: IChatParticipant | any): void {
  ...
  private getClosestWindow(window: Window): Window | undefined | any
  {
  ...
  onWindowMessagesSeen(messagesSeen: Message[] | any): void {
      this.markMessagesAsRead(messagesSeen);
  }

  onWindowChatClosed(payload: { closedWindow: Window, closedViaEscapeKey: boolean } | any): void {
  ...
  onWindowTabTriggered(payload: { triggeringWindow: Window, shiftKeyPressed: boolean } | any): void {
  ...
  onWindowMessageSent(messageSent: Message | any): void {
      this.adapter.sendMessage(messageSent);
  }

  onWindowOptionTriggered(option: IChatOption | any): void {
      this.currentActiveOption = option;
  }

  triggerOpenChatWindow(user: User | any): void {
      if (user)
      {
